...
public interface AppoinmentDetailsRepo extends JpaRepository<AppointmentDetails, String> {
	@Query(value = "SELECT ad.id, dd.first_name, dd.last_name, dd.mobile_no, dd.email_id, dd.chembar_address, appsc.schedule_day, appsc.schedule_time_start,appsc.schedule_time_end,ad.appointment_confrimed FROM appointment_details ad,doctor_details dd,appointment_schedule appsc WHERE ad.doctor_id=dd.id AND ad.appointment_schedule_id=appsc.id AND ad.patient_id=?", nativeQuery = true)
